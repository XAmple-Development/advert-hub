  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-pink-500 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-pulse delay-150"></div>
      </div>

      <div className="flex items-center justify-center min-h-[calc(100vh-64px)] p-4 relative z-10">
        <div className="w-full max-w-6xl flex gap-8">
          {/* Main Auth Card */}
          <Card className="flex-1 max-w-md mx-auto bg-gradient-to-br from-slate-800/90 to-slate-900/90 border-slate-700/50 backdrop-blur-xl shadow-2xl">
            <CardHeader className="text-center pb-8">
              <div className="w-16 h-16 bg-gradient-to-br from-purple-500 to-pink-500 rounded-2xl mx-auto mb-4 flex items-center justify-center">
                <Bot className="w-8 h-8 text-white" />
              </div>
              <CardTitle className="text-white text-3xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
                Discord Boost
              </CardTitle>
          ...
          {/* Discord Tools Panel - Only show if user is authenticated */}
          {user && (
              ...
              <Card className="bg-gradient-to-br from-slate-800/90 to-slate-900/90 border-slate-700/50 backdrop-blur-xl shadow-2xl">
                <CardHeader>
                  ...
                  <CardDescription className="text-slate-400">
                    Import your servers and manage your bot
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  <Button
                    onClick={() => setShowDiscordImport(true)}
                  ...
                  >
                    Import Discord Servers
                  </Button>
                  <Button
                    onClick={() => setShowBotManagement(true)}
                    variant="outline"
                    className="w-full border-slate-600 text-slate-300 hover:bg-slate-700 hover:text-white font-semibold transition-all duration-300"
                  >
      ...
      <DiscordImportModal
        open={showDiscordImport}
        onOpenChange={setShowDiscordImport}
        onImportComplete={() => {
          setShowDiscordImport(false);
          toast({
            title: "Import completed!",
            description: "Your Discord servers have been imported successfully.",
          });
        }}
      />
      ...
      {showBotManagement && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 max-w-4xl w-full max-h-[80vh] overflow-y-auto border border-slate-700">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-white">Bot Management</h2>
              <Button
                onClick={() => setShowBotManagement(false)}
                variant="ghost"
