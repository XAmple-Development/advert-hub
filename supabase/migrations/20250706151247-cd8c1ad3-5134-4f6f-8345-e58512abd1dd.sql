...
CREATE TABLE public.votes (
  ...
  ip_address TEXT,
  UNIQUE(user_id, target_id, date_trunc('day', voted_at))
);
...
ALTER TABLE public.forum_categories ADD CONSTRAINT forum_categories_sort_order_check CHECK (sort_order >= 0);
...
CREATE INDEX idx_votes_user_daily ON public.votes(user_id, date_trunc('day', voted_at));
...
BEGIN
    ...
    -- Update vote counts
    IF p_target_type = 'server' OR p_target_type = 'bot' THEN
...
AS $$
BEGIN
    INSERT INTO public.user_reputation (user_id, reputation_score, total_votes_received, total_listings)
    SELECT 
        l.user_id,
        COALESCE(l.vote_count, 0) * 2 + COALESCE(l.bump_count, 0),
        COALESCE(l.vote_count, 0),
        1
    ...
    DO UPDATE SET
        reputation_score = (
            SELECT SUM(COALESCE(vote_count, 0) * 2 + COALESCE(bump_count, 0))
            FROM public.listings 
            WHERE user_id = user_reputation.user_id
        ),
        total_votes_received = (
            SELECT SUM(COALESCE(vote_count, 0))
            FROM public.listings
            WHERE user_id = user_reputation.user_id
        ),
        total_listings = (
            SELECT COUNT(*)
            FROM public.listings
            WHERE user_id = user_reputation.user_id AND status = 'active'
        ),
        updated_at = now();
...
-- Trigger to update reputation when listings change
