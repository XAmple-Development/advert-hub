-- Fix remaining critical security issues (avoiding duplicate policies)
...
-- Only create policies if they don't exist (server_verification policies might already exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'server_verification' 
    AND policyname = 'Admins can manage server verification'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'server_verification' 
    AND policyname = 'Public can view verified servers'
  ) THEN
